...
**Item grounding: done** (2026-08-10). The other half of invention, and the half a number can't settle. Spec §12.1 prescribes "pack guardrails reject; fail closed" for a hallucinated offering, and only the price half was implemented: `check_guardrails` asks that the slot's offering *is named*, never that nothing else was added, so "Chocolate truffle cake ₹550 and fresh butter croissants" passed everything. Each pack now declares `Guardrails.item_lexicon` — the item vocabulary of its vertical — and `agents/common.find_unstocked_item` rejects any of those nouns no offering of *that client* covers. Lexicon = the vertical's words (pack), stocked = what this shop sells (Client Context); a bakery selling "Butter croissant" is unaffected by `croissant` being listed. **Default empty, unlike `require_price_grounding`** — the engine cannot guess a vertical's nouns, so a pack without a lexicon honestly gets no item protection. Matching is word-boundary + simple plural (`cake` does not fire inside `cheesecake`, so packs list both), and the business name is stripped first so "The Nail Bar" can't trip its own lexicon. Applied to **captions and broadcasts only**: review replies are exempt on purpose, because a reply may legitimately echo a reviewer asking for something the shop doesn't stock, and dropping that reply is the wrong call. Both call sites already retry once with the reason, so a false positive gets a second attempt. Red-team case `rt_invented_item` pins it. Also: content drafts now record `meta["offering"]` (previously discarded with the `Slot`) and the owner's digest and `LIST` show `[id] date · Chocolate truffle cake` above the caption — the lexicon only knows anticipated nouns, so the owner's read stays the backstop and they can now see what the post was *meant* to be about.
...
**Known limits of the eval harness** (`docs/evals.md` has the full version): semantic tone isn't scored — `brand_voice` measures register (hype, shouting, speaking as the shop), not whether a caption *feels* homely; and **an invented item the pack's lexicon didn't anticipate is still caught at swap time or not at all**. Runtime now covers prices (any amount) and items a pack listed, but no list of nouns is complete, so "fresh butter danishes" gets through if `danish` isn't in the lexicon — the gap narrowed from "any invented item" to "any invented item nobody thought of". `BAKERY_NOT_SOLD`'s `biryani` is deliberately outside the bakery lexicon so `tests/test_evals.py` can keep pinning what only the harness sees. The lexicons are also English-only, so a Marathi caption transliterating an item name is unchecked — same root cause as the non-English grounding weakness below. That's still the argument against `AUTO ON gbp_post` for a model that hasn't cleared the grounding dimension, and it sharpens when GBP publishing stops being semi-manual, because the owner's copy-paste into Google is currently the last human read of a caption. Non-English grounding is also weak: offering names are stored in English and matched as substrings, so a good Marathi caption that transliterates the item name gets rejected by the engine (surfacing as `coverage`, not `language`) — fix before any Marathi-first pilot.
